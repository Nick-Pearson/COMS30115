...
float Material::CalculateBRDF(const glm::vec3& view, const glm::vec3& light, const glm::vec3& normal)
{
...
float Material::Distribution(const glm::vec3& h, const glm::vec3& normal)
{
...
float Material::Fresnel(const glm::vec3& view, const glm::vec3& h, const glm::vec3& normal)
{
  const float Fzero = 0.1f;
  const float dot = glm::dot(view, h);
  const float i = ((-5.55473f * dot) - 6.98316f) * dot;
  return Fzero + ((1.0f - Fzero) * pow(2.0f, i));
}

float Material::Geometry(const glm::vec3& light, const glm::vec3& view, const glm::vec3& normal)
{
  return Geometry_Internal(light, normal) * Geometry_Internal(view, normal);
}

float Material::Geometry_Internal(const glm::vec3& vector, const glm::vec3& normal)
{
